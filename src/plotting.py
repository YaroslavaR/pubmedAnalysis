...
def create_amount_of_article_citations_per_year_plot(term1_map, term2_map, term1_cit_map, term2_cit_map):
	plt.figure(1, figsize=(10,5))
	plt.subplot(211).xaxis.set_major_formatter(tick.FormatStrFormatter('%0.0f'))
	plt.plot(list(term1_map.keys()),list(term1_map.values()), ':r', label=SEARCH_TERM_2)
	plt.plot(list(term2_map.keys()),list(term2_map.values()), '--c', label=SEARCH_TERM_1)
	...
	plt.plot(list(term1_cit_map.keys()),list(term1_cit_map.values()), ':r', label=SEARCH_TERM_2)
	plt.plot(list(term2_cit_map.keys()),list(term2_cit_map.values()), '--c', label=SEARCH_TERM_1)
...
def create_avg_citations_age_per_year_plot(term1_cit_map, term2_cit_map, term1_age_of_cited_work, term2_age_of_cited_work):
	plt.figure(2, figsize=(10,5))
	plt.subplot(211).xaxis.set_major_formatter(tick.FormatStrFormatter('%0.0f'))
	plt.plot(list(term1_cit_map.keys()),avg(list(term1_cit_map.values())),':r', label=SEARCH_TERM_2)
	plt.plot(list(term2_cit_map.keys()),avg(list(term2_cit_map.values())),'--c', label=SEARCH_TERM_1)
	...
	plt.bar(ind_ai-0.15, avg_list(term1_age_of_cited_work.values()),0.3, color='r', label=SEARCH_TERM_2)
	plt.bar(ind_ml+0.15, avg_list(term2_age_of_cited_work.values()), 0.3, color='c', label=SEARCH_TERM_1)
